...
import GP_model_6configs_fr293_sweep_updated as f
import re
...
def computestrain(position_stack, force_stack, i):
    ...
    # avoid an error generated by a zero length position vector for the initial point
    force_magnitude_stack[0] = np.linalg.norm(force_stack[0, :])
    for k in range(1, i+1):
        force_magnitude_stack[k] = np.linalg.norm(force_stack[k, 0:3])
        unit_force = force_stack[k, 0:3] / force_magnitude_stack[k]
...
def findblobstack(image_filename, image_filepath, output_filepath, ca, cc, cropsize, sigma, gamma_adjust):
    ...
    try:
        time_stack = np.genfromtxt(image_filepath + image_filename + '_time.csv', dtype=float, delimiter=',')
    except IOError:
    ...
    z_measured = z_values[0]-z_values[1]
    ...
    for i in tqdm(range(0, length)):
        ...
        circ = Circle(position_stack[i, :], sigma, color='red', linewidth=2, fill=False)
        arrow = Arrow(x, y, 100 * force_stack[i, 0], 100 * force_stack[i, 1], width=30, color='black')
        ax2.add_patch(circ)
        if force_on:
            ax2.add_patch(arrow)
        ax2.scatter(x=position_stack[0:i, 0], y=position_stack[0:i, 1], c='g', s=10)
