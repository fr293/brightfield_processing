...
def gp_model(no):
	...
	gpx = gaussian_process.GaussianProcess(corr='squared_exponential', optimizer='fmin_cobyla', theta0=1e-2,  storage_mode = 'light', thetaL=1e-2, thetaU=100, nugget=0.1)
	gpy = gaussian_process.GaussianProcess(corr='squared_exponential', optimizer='fmin_cobyla', theta0=1e-2, storage_mode = 'light', thetaL=1e-2, thetaU=100, nugget=0.1)
	gpz = gaussian_process.GaussianProcess(corr='squared_exponential', optimizer='fmin_cobyla', theta0=1e-2,  storage_mode = 'light', thetaL=1e-2, thetaU=100, nugget=0.1) 
	...
	data_list = objective_function(no)
	...
	joblib.dump(gpx,'config'+cc+'x.pkl')
	joblib.dump(gpy,'config'+cc+'y.pkl')
	joblib.dump(gpz,'config'+cc+'z.pkl')
	#joblib.dump(gpz,'../../Data/Exp4_20151102/GP_model/config'+cc+'f.pkl')
	
	print("Regression Done")

################# Making Predictions  ###################
def prediction(no,x,y,z):
	...
	#reading data from the files
	#csv_in1 = csv.reader(open('/media/qc230/data-d2/Qian/Calibration/Data/Exp4_20151102/Exp_Origin/laps_traj_full_2015_12_18_E1_DNROCK_force.csv', 'rb'), delimiter = ',') 
	...
	####################Gaussian Process (GP)######################
	gp1x = joblib.load('config'+cc+'x.pkl')
	gp1y = joblib.load('config'+cc+'y.pkl')
	gp1z = joblib.load('config'+cc+'z.pkl')
	
	#gp4x = joblib.load('../../Data/Exp6_20160104/GP_model/config4x.pkl')
	#gp4y = joblib.load('../../Data/Exp6_20160104/GP_model/config4y.pkl')
	#gp4z = joblib.load('../../Data/Exp6_20160104/GP_model/config4z.pkl')
	
	########The below starts the prediction process ################
	
	#for i in range(len(rawdata)):
		
	#	x = float(rawdata[i][8]) * 1.e-3
	#	y = float(rawdata[i][9]) * 1.e-3 
	#	z = float(rawdata[i][10])* 1.e-3
	...
	gpz, sigma3 = gp1z.predict([x,y,z], eval_MSE=True)
		...
		#elif rawdata[i][6] == 'down':
			
		#	gpx, sigma1 = gp4x.predict([x,y,z], eval_MSE=True)
			#gpy, sigma2 = gp4y.predict([x,y,z], eval_MSE=True)
			#gpz, sigma3 = gp4z.predict([x,y,z], eval_MSE=True)
		
	gpx = float(gpx)
	gpy = float(gpy)
	gpz = float(gpz)
		
		#newdata.append([rawdata[i][0], rawdata[i][1], rawdata[i][2], rawdata[i][3], rawdata[i][4], rawdata[i][5], rawdata[i][6], rawdata[i][7], x*1e3, y*1e3, z*1e3, gpx, gpy, gpz, rawdata[i][14], rawdata[i][15]])
		
	#with open('/media/qian/data-d2/Qian/Calibration/Data/Exp6_20160104/Exp_Origin/laps_traj_full_2015_12_18_E1_DNROCK_force_QC.csv', "wb") as f:
...
def validation(no):
	...
	#reading data from the files
	csv_in1 = csv.reader(open('/group/data-d2/Qian/Calibration/Data/Exp4_20151102/Config'+cc+'/valid.csv', 'rb'), delimiter = '\t') 
...
def data_select():
	...
	#reading data from the files
	csv_in1 = csv.reader(open('/media/qc230/data-d2/Qian/Calibration/Data/Exp4_20151102/Config1/Config1.csv', 'rb'), delimiter = '\t') 
	...
	for i in range(len(x1)):
		...
		traindata.append([x1[i][0]*1.e-3, x1[i][1]*1e-3, x1[i][2]*1e-3, x1[i][3]*1., x1[i][4]*1., x1[i][5]*1., force])
...
def data_selection(no):
	...
	#reading data from the files
	csv_in1 = csv.reader(open('/media/qc230/data-d2/Qian/Calibration/Data/Exp4_20151102/Config'+cc+'/Config'+cc+'.csv', 'rb'), delimiter = '\t') 
	...
	for i in range(len(x1)):
		...
		if i == 0:
			...
			traindata.append([x1[n][0]*1.e-3, x1[n][1]*1e-3, x1[n][2]*1e-3, x1[n][3]*1., x1[n][4]*1., x1[n][5]*1., x1[n][6]*1.])
		...
		if abs(x1[i][1] - x1[n][1]) >= 40 and abs(x1[i][0] - x1[n][0]) <= 41 :
			...
			traindata.append([x1[m][0]*1.e-3, x1[m][1]*1e-3, x1[m][2]*1e-3, x1[m][3]*1., x1[m][4]*1., x1[m][5]*1., x1[m][6]*1.])
			traindata.append([x1[n][0]*1.e-3, x1[n][1]*1e-3, x1[n][2]*1e-3, x1[n][3]*1., x1[n][4]*1., x1[n][5]*1., x1[n][6]*1.])
	...
	with open('/media/qc230/data-d2/Qian/Calibration/Data/Exp4_20151102/Config'+cc+'/Config'+cc+'_selected3.csv', "wb") as f:
...
def data_selection2(no):
	...
	#reading data from the files
	csv_in1 = csv.reader(open('/media/qian/data-d2/Qian/Calibration/Data/Exp6_20160104/Config'+cc+'/Config'+cc+'_selected.csv', 'rb'), delimiter = '\t') 
	...
	with open('/media/qian/data-d2/Qian/Calibration/Data/Exp6_20160104/Config'+cc+'/Config'+cc+'_selected2.csv', "wb") as f:
...
def checkexp():
	...
	#rawdata1=[]
	#reading data from the files
	csv_in1 = csv.reader(open('/group/data-d2/Qian/Calibration/Data/Exp2_20150730/Config4/training.csv', 'rb'), delimiter = '\t') 
...
def validation(no):
	...
	#reading data from the files
	csv_in1 = csv.reader(open('/group/data-d2/Qian/Calibration/Data/Exp2_20150722/Config'+cc+'/valid.csv', 'rb'), delimiter = '\t') 
