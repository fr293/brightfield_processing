...
def objective_function(filename):
    name = str(filename)
...
def gp_model(path, filename, no):
    ...
    data_list = objective_function(os.path.join(path_str, name))
...
def prediction(location_array, predictor_array_x, predictor_array_y, predictor_array_z):

    sx = [0]
    sy = [0]
    sz = [0]

    gpx = predictor_array_x
    gpy = predictor_array_y
    gpz = predictor_array_z
    ...
    force_stack = np.concatenate((mx, my, mz), axis=1)
